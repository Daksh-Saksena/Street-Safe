import math
import time
from pymavlink import mavutil
class MAVLinkInterface:
    def __init__(self, conn, baud=115200):
        self.conn = conn
        self.baud = baud
        self.vehicle = None
        self._heartbeat = 0.0
    def connect(self):
        try:
            self.vehicle = mavutil.mavlink_connection(self.conn, baud=self.baud)
            self.vehicle.wait_heartbeat(timeout=10)
            self._heartbeat = time.time()
            return True
        except Exception as e:
            print(f"MAVLink connect failed: {e}")
            return False
    def is_connected(self):
        if self.vehicle is None:
            return False
        return (time.time() - self._heartbeat) < 5.0
    def update_heartbeat(self):
        if self.vehicle is None:
            return
        msg = self.vehicle.recv_match(blocking=False)
        if msg and msg.get_type() == "HEARTBEAT":
            self._heartbeat = time.time()
    def disconnect(self):
        if self.vehicle is not None:
            self.vehicle.close()
            self.vehicle = None
    def get_gps(self):
        if not self.is_connected():
            return None
        try:
            msg = self.vehicle.recv_match(type="GPS_RAW_INT", blocking=True, timeout=3)
            if msg is None:
                return None
            return {
                "lat": msg.lat / 1e7,
                "lon": msg.lon / 1e7,
                "alt_msl": msg.alt / 1e3,
                "fix_type": msg.fix_type,
                "satellites_visible": msg.satellites_visible,
            }
        except Exception as e:
            print(f"get_gps error: {e}")
            return None
    def get_altitude(self):
        if not self.is_connected():
            return None
        try:
            msg = self.vehicle.recv_match(type="VFR_HUD", blocking=True, timeout=3)
            if msg is None:
                return None
            return {
                "alt_relative": msg.alt,
                "groundspeed": msg.groundspeed,
                "airspeed": msg.airspeed,
                "heading": msg.heading,
                "throttle": msg.throttle,
            }
        except Exception as e:
            print(f"get_altitude error: {e}")
            return None
    def get_attitude(self):
        if not self.is_connected():
            return None
        try:
            msg = self.vehicle.recv_match(type="ATTITUDE", blocking=True, timeout=3)
            if msg is None:
                return None
            return {
                "roll": math.degrees(msg.roll),
                "pitch": math.degrees(msg.pitch),
                "yaw": math.degrees(msg.yaw),
                "rollspeed": math.degrees(msg.rollspeed),
                "pitchspeed": math.degrees(msg.pitchspeed),
                "yawspeed": math.degrees(msg.yawspeed),
            }
        except Exception as e:
            print(f"get_attitude error: {e}")
            return None
    def send_waypoint(self, lat, lon, alt):
        if not self.is_connected():
            return False
        try:
            self.vehicle.mav.mission_item_int_send(
                self.vehicle.target_system,
                self.vehicle.target_component,
                0,
                mavutil.mavlink.MAV_FRAME_GLOBAL_RELATIVE_ALT_INT,
                mavutil.mavlink.MAV_CMD_NAV_WAYPOINT,
                2, 1,
                0, 0, 0, float("nan"),
                int(lat * 1e7),
                int(lon * 1e7),
                alt,
            )
            return True
        except Exception as e:
            print(f"send_waypoint error: {e}")
            return False
    def set_velocity(self, vx, vy, vz):
        if not self.is_connected():
            return False
        try:
            mask = (
                mavutil.mavlink.POSITION_TARGET_TYPEMASK_X_IGNORE
                | mavutil.mavlink.POSITION_TARGET_TYPEMASK_Y_IGNORE
                | mavutil.mavlink.POSITION_TARGET_TYPEMASK_Z_IGNORE
                | mavutil.mavlink.POSITION_TARGET_TYPEMASK_AX_IGNORE
                | mavutil.mavlink.POSITION_TARGET_TYPEMASK_AY_IGNORE
                | mavutil.mavlink.POSITION_TARGET_TYPEMASK_AZ_IGNORE
                | mavutil.mavlink.POSITION_TARGET_TYPEMASK_YAW_IGNORE
                | mavutil.mavlink.POSITION_TARGET_TYPEMASK_YAW_RATE_IGNORE
            )
            self.vehicle.mav.set_position_target_local_ned_send(
                0,
                self.vehicle.target_system,
                self.vehicle.target_component,
                mavutil.mavlink.MAV_FRAME_LOCAL_NED,
                mask,
                0, 0, 0,
                vx, vy, vz,
                0, 0, 0,
                0, 0,
            )
            return True
        except Exception as e:
            print(f"set_velocity error: {e}")
            return False
    def set_guided_mode(self):
        if not self.is_connected():
            return False
        try:
            self.vehicle.set_mode("GUIDED")
            return True
        except Exception as e:
            print(f"set_guided_mode error: {e}")
            return False
    def arm(self):
        if not self.is_connected():
            return False
        try:
            self.vehicle.arducopter_arm()
            self.vehicle.motors_armed_wait()
            return True
        except Exception as e:
            print(f"arm error: {e}")
            return False
